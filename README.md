...
The Dæmon Saigo project makes it possible to rebuild the Native Client Software Development Kit for use with the [Dæmon game engine](https://github.com/DaemonEngine/Daemon).
...
Google never compiled the Saigo SDK for anything other than Linux on amd64,
and their build process relied on a very complex collection of repositories involving the execution of prebuilt binaries.
This project not only makes it possible to rebuild Saigo for your preferred system and architecture,
but does so without running any shady precompiled executable provided by Google.
...
This project also ships with the compilers and debugger and some NaCl C/C++ headers historically stored in the runtime repository:
...
That other project makes possible to rebuild the NaCl runtime with our own fixes and without Google's complex collection of repositories and without Google's shady precompiled binaries.
...
For now, the libc and libc++ libraries are copied from precompiled libraries provided by Google.
...
meaning no precompiled code runs in the trusted environement outside of the Native Client sandbox.
...
The supported operating systems to build the Saigo toolchain on are Linux, FreeBSD and macOS.
...
Those build scripts and CMake configuration are only tested in Unix-like environments.
...
- `git`
- A C/C++ compiler collection for the target, the following are supported:
...
The provided CMake script driving the compilation of all Saigo components deletes useless files after compilation and replace known duplicates with symbolic links.
...
It is highly recommended to share the repository over the network accross all the build machines to let the build tasks reuse things as much as possible.
...
Beware that building using  LTO is much slower and can require a crazy amount of RAM.
To prevent the kernel to trigger the OOM killer and to preserve your precious uptime,
the `build` helper sets the linker job count accordingly to both available CPU cores and available memory.
It works best when the Ninja build tool is used.

The `build` task makes heavy use of symbolic links to deduplicates files (see above).
...
All symbolic links are turned into hardlinks in the Windows tarballs to both provide an efficient storage and make sure files are extracted as real files and not as broken links on Windows.
...
which signs the checksum file for the packages using the signing key configured for the Git repository.
...
History of NaCl toolchains, from newest to oldest (only the latest version of them being mentioned):
...
The latest version of Saigo is based on LLVM Clang 21 and as such is expected C23 support and likely partial C2y draft support, C++23 support and likely partial C++2c draft support.

The libc++ is the one from the same LLVM Saigo is based on, and should be easy to build.
Unfortunately building some parts of the libc still requires the old NaCl GCC, which is now very old and very hard to build, and Google itself had given up and did not rebuild it with their scripts.

Out of convenience, the libc++ and libc are currently packaged from prebuilt packages by Google.
The libc and the libc++ are linked to binaries running in the Native Client virtual machine, and as such, never run outside of the sandbox.
...
The purpose of this project is to provide a way to build native binaries for the toolchain running on developer's computer without:

- using Google-provided binaries to build NaCl code, that means Clang and Binutils are rebuilt from scratch;
- using Google-provided binaries to rebuild the toolchain itself, that means no Google binaries are used to build Clang and Binutils themselves.

This also allows to run the toolchain on systems not supported by Google.
Google only built Saigo Clang for `linux-amd64`, and Google only built PNaCl Clang for `linux-amd64`, `windows-amd64`, `windows-i686` and `macos-amd64`.

The Saigo toolchain can only build `nexe` applications for `amd64`, `i686` and `armhf`.
While there are no Native Client loaders for other architectures and other systems than Linux, Windows and macOS,
those `nexe` applications and loaders can run on architecture and system variants using compatibility layers.
Those build scripts make it possible to build and run a native Saigo toolchain on the same systems the NaCl loader is known to run either natively or through known compatibility layers.
...
For the nexe libc and libc++, this project repackages the Google precompiled libraries in order to reach a minimum viable product state.
...
Contributions making it possible to fully rebuild the libc without any Google-provided binaries are welcome,
...
and the NaCl loader can be built for running the `nacl-mipsel` target, but neither Saigo nor its related Binutils can be built for it.
No prebuilt `nacl-mipsel` libc/libc++ to be used with Saigo has ever been distributed by Google.
...
There are some exception-related files provided here and there with Dæmon Saigo, but the feature has not been seen working.
...
Support for exceptions seems to have been dropped a decade before Google deprecated NaCl in Chromium technologies.

Contributions to restore the exception support will be much appreciated.
